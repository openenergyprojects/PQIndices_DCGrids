...
% 2010) eq. (3) @ pp. 1112
function [EnDominantFreq,dominantFreq]= fftIna_1(u, Fs, signalDuration, k, signalName)
...
title ('FFT of signal', 'fontsize',  titleFontSize, 'fontweight', 'normal');
xlabel('frequency (Hz)', 'FontSize',labelFontSize);
ylabel('Signal Amplitude (V)','FontSize',labelFontSize);
...
title('Zoom on FFT analysis on half mirror', 'fontsize',  titleFontSize, 'fontweight', 'normal');
xlabel('frequency (Hz)','FontSize',labelFontSize);
ylabel('Signal Amplitude (V)','FontSize',labelFontSize);
% figure(7);
...
subplot(2,2,3:4);
spectrogram(u, window, noverlap, Nfft, Fs,'yaxis');
title ('spectrogram', 'fontsize',  titleFontSize, 'fontweight', 'normal');
...
%  text( 0.5, 0, 'PQ indices for Frequency Domain analysis', 'FontSize', 12', ...
